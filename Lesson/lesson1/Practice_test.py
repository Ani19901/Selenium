...
def test():
    ...
    chrome_search = ChromePage(driver_obj.driver)
    ...
    try:
        ...
        chrome_search.fill_in_search_field("Automation")
        get_logging.info(f"Get Search result")
        result_count = chrome_search.get_result_count()
...
# no need to input logs in test cases, try to input in functions\
# POM structure with folders is not kept
# you should have pages, tests folders and helpers.py, whereinclude all universal functions
# files names are not to the point, give pages names with UI names, keep lib or helper to file where include universal functions
# need to create waiting function for all buttons

# not bad, need to refactore a bit.